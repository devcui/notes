...
function makeDefaults() {
  const defaults = {
     ...
     *
     * description: Enables HTTP POST Method support at the Authorization Endpoint and the Logout Endpoint (if enabled).
     * This setting can only be used when the `cookies.long.sameSite` configuration value is `none`.
     */
...
- After running merge, all properties are in config
...
- Most of the remaining function calls are to check parameters
...
- Get the storage function and OAUTH client from the configuration
...
- Add OIDC core functionality as middleware to your Koa application
...
export default function initializeApp() {
  ...
  if (features.dPoP.enabled && features.dPoP.nonceSecret) {
...
- Then register the endpoint (route)
...
- The core of the authorization endpoint consists of a middleware stack composed of different configuration combinations, which are then called in sequence.

## The rest

- There are a lot of codes but most of them are about the following
- The rest is to follow the oidc process
	- Parameter verification -> return result
	- Parameter verification -> view config -> call adapter storage -> return result


Well, that’s the end of the entrance and process of oidc. Thanks for reading. The rest is up to you.